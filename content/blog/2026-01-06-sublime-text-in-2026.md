...
Sublime Text uses the old TextMate approach to syntax definitions (.tmLanguage), as does VS Code, evolved to the newer .sublime-syntax. Both use regex to match parts of the code to specific scopes. Those scopes can then be styled by color schemes (.sublime-color-scheme) which are based on, and evolved, from the TextMate .tmTheme format. It's very easy to find what scope has been applied and then to edit the color scheme to taste.
